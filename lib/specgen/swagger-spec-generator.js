...
module.exports = function createSwaggerObject(loopbackApplication, opts) {
  ...
  swaggerObject.tags = _.orderBy(swaggerObject.tags,'name','asc');
...
function generateSwaggerObjectBase(opts, loopbackApplication) {
  ...
  var filepath = path.join(process.cwd(),'DESCRIPTION.md');
    console.log('DEBUGGING MADNESS',filepath, fs.existsSync(filepath))
  if (fs.existsSync(filepath)) {
      apiInfo.description = fs.readFileSync(filepath, { encoding: 'utf8' });
  }
